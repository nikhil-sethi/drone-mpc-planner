...
[ -f dependencies-packages.done ] || {
	...
	sudo apt-get update
	sudo apt install -y cmake g++ libva-dev libswresample-dev libavutil-dev pkg-config libcurl4-openssl-dev ncdu openssh-server ffmpeg astyle unattended-upgrades inotify-tools 
