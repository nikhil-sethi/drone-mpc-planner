...
def def_optimization():
    ...
    objective = delta_t[:, :]
    objective += 1e13*casadi.sum1(casadi.sum2(virtual_inputs[:, :]**2))
    objective += 1e15*casadi.sum1(final_pos_error_slack[:,:]**2)
    objective += 1e15*final_angle_error_slack[:,:]**2
    ...
    print(type(constr_dronedynamicI))
    ...
    ### Direction:
    constr_intercepting_anglerr = casadi.dot(inputs[idx_acc, -1], insect_states[idx_pos, -2] - states[idx_pos, -2]) \
                                   / casadi.norm_2(insect_states[idx_pos, -2] - states[idx_pos, -2]) \
                                   / casadi.norm_2(inputs[idx_acc, -1]) \
...
def gen_initguess(x0, u0, xinsect, uend_guess):
    ...
    xend = np.array([insect_guess[-1, 0], insect_guess[-1, 1], insect_guess[-1, 2], drone_velEnd[0], drone_velEnd[1], drone_velEnd[2]])
    statesI_guess = np.linspace(x0, xend, n_samples, axis=1).T
    inputsI_guess = np.linspace(u0, uend_guess, n_samples-1, axis=1).T
