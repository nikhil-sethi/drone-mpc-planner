...
void InterceptInPlanesQuadraticOptimizer::init() {
    solver = new SQProblem(65, 47);

    Options options;
    options.setToMPC();
    options.printLevel = PL_NONE;
    options.enableRegularisation = BT_TRUE;
    Eigen::VectorXd xopt = Eigen::VectorXd(65);
...
Eigen::VectorXd InterceptInPlanesQuadraticOptimizer::constraints(Eigen::VectorXd xopt, Eigen::VectorXd param [[maybe_unused]]) {
    ...
    constraints[21] =  (((xopt[35]+((xopt[34]*(xopt[38]+(xopt[38]+(xopt[34]*xopt[47]))))/const1))-xopt[41])+xopt[50]);
    constraints[22] =  (((xopt[36]+((xopt[34]*(xopt[39]+(xopt[39]+(xopt[34]*xopt[48]))))/const1))-xopt[42])+xopt[51]);
    constraints[23] =  (((xopt[37]+((xopt[34]*(xopt[40]+(xopt[40]+(xopt[34]*xopt[49]))))/const1))-xopt[43])+xopt[52]);
    constraints[24] =  (((xopt[38]+((xopt[34]*(xopt[47]+xopt[47]))/const1))-xopt[44])+xopt[53]);
    constraints[25] =  (((xopt[39]+((xopt[34]*(xopt[48]+xopt[48]))/const1))-xopt[45])+xopt[54]);
    constraints[26] =  (((xopt[40]+((xopt[34]*(xopt[49]+xopt[49]))/const1))-xopt[46])+xopt[55]);
...
Eigen::MatrixXd InterceptInPlanesQuadraticOptimizer::constraint_derivative(problem_parameters *prob_params, problem_solution *prev_qpsolution) {
    Eigen::VectorXd xopt = prev_qpsolution->Xopt;
    Eigen::VectorXd param = prob_params->param;
    Eigen::MatrixXd dconstraints = Eigen::MatrixXd(47, 65);
