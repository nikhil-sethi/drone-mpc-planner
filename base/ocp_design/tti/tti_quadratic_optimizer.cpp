...
void TtiQuadraticOptimizer::init() {
    solver = new SQProblem(34, 15);

    Options options;
    options.setToMPC();
    options.printLevel = PL_NONE;
    options.enableRegularisation = BT_TRUE;
    Eigen::VectorXd xopt = Eigen::VectorXd(34);
...
Eigen::VectorXd TtiQuadraticOptimizer::constraints(Eigen::VectorXd xopt, Eigen::VectorXd param [[maybe_unused]]) {
    Eigen::VectorXd constraints = Eigen::VectorXd(15);
    double const1 = 2;
    ...
    constraints[6] =  ((xopt[22]+((xopt[0]*(xopt[25]+xopt[25]))/const1))-xopt[28]);
    constraints[7] =  ((xopt[23]+((xopt[0]*(xopt[26]+xopt[26]))/const1))-xopt[29]);
    constraints[8] =  ((xopt[24]+((xopt[0]*(xopt[27]+xopt[27]))/const1))-xopt[30]);
...
Eigen::MatrixXd TtiQuadraticOptimizer::constraint_derivative(problem_parameters *prob_params, problem_solution *prev_qpsolution) {
    Eigen::VectorXd xopt = prev_qpsolution->Xopt;
    Eigen::VectorXd param = prob_params->param;
    Eigen::MatrixXd dconstraints = Eigen::MatrixXd(15, 34);
