...
/usr/bin/screen -t upc /bin/bash -c 'cd ~/pats/release/scripts && ~/pats/release/scripts/auto_daemon_starter.py && sleep 1; exec /bin/bash'
...
[ -f /home/pats/pats/flags/disable_trapeye ] || {
	/usr/bin/screen -t m1 /bin/bash -c '~/pats/release/build/trapeye'
	/usr/bin/screen -t m2 /bin/bash -c '~/pats/release/scripts/run_n_trapeye.sh 30'
}
