    ...
    switch (_state) {
        case ds_pre_flight: {
                ...
                if (_interceptor->control_mode() == acceleration_feedforward)
                    control.control(state, nav.setpoint(),  acceleration_feedforward, time, true);
                else if (_interceptor->control_mode() == velocity_control)
                    control.control(state, nav.setpoint(), velocity_control, time, true);
                else if (_interceptor->control_mode() == acceleration_control)
                    control.control(state, nav.setpoint(), acceleration_control, time, true);
                else
                    control.control(state, nav.setpoint(), position_control, time, true);
