...
void DroneController::control(TrackData data_drone, TrackData data_target, control_modes control_mode, double time, bool enable_logging) {
    ...
    switch (_flight_mode) {
        case fm_manual: {
                ...
                remaining_spinup_duration_t0 = max(0.f, dparams.full_bat_and_throttle_spinup_duration);
#else
                remaining_spinup_duration_t0 = 0.f;
...
void DroneController::mix_drone_accelerations(TrackData data_drone, cv::Point3f target_acc) {
    ...
    if (normf(kiv_acc) > 0) {
        ...
        acceleration_mix = combine_drone_accelerations_with_priority(acceleration_mix, component_a_perpendicular_to_b(target_acc, kiv_acc)); // FIXME: what's the purpose of this component_a_perpendicular_to_b magic?
...
bool DroneController::abort_take_off() {
    ...
    if (_flight_mode == fm_start_takeoff || _flight_mode == fm_remaining_spinup) {
        _flight_mode = fm_start_takeoff;
        return true;
    } else
        return false;
}
