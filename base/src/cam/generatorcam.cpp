...
StereoPair *GeneratorCam::update() {
    if (_rc->throttle > RC_BOUND_MIN || takeoff_start_time > 0) {
        ...
        if (dt_takeoff < 0.2f && dt_takeoff > 0)
            current_drone_pos = drone_start_pos + cv::Point3f(0, 0.2f * dparams.max_thrust * powf(dt_takeoff, 2), 0);
        else if (dt_takeoff < 0)
            current_drone_pos = drone_start_pos + cv::Point3f(0, 0.2f * dparams.max_thrust * powf(0, 2), 0);
        else if (dt_takeoff < 1)
            current_drone_pos = drone_start_pos + cv::Point3f(0, 0.2f * dparams.max_thrust * powf(0.2, 2), 0);
        else if (dt_takeoff < 2) {
            auto pos1 = drone_start_pos +  cv::Point3f(0, 0.2f * dparams.max_thrust * powf(0.2, 2), 0);
