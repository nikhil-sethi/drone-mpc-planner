...
void Interceptor::update_aim_in_flightarea(rapid_route_result rapid_route_res) {
    if (rapid_route_res.valid) {
        ...
        if (_flight_area->inside(rapid_route_res.position_to_intercept, strict))
            interception_position_in_flightarea = true;
        else
            interception_position_in_flightarea = false;
        if (_flight_area->inside(rapid_route_res.stopping_position, strict))
    ...
    for (auto trkr : all_trackers) {
        if (trkr->tracking()) {
            ...
            if (_optim_result.valid) {
                bool aim_inview = _flight_area->inside(_optim_result.position_to_intercept, strict);
                bool stop_inview = _flight_area->inside(_optim_result.stopping_position, strict);
