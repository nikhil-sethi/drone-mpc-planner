...
void Visualizer::add_plot_sample(void) {
    if (enable_plots) {
        ...
        throttle_min_bound.push_back(static_cast<float>(BF_CHN_MIN));
        throttle_max_bound.push_back(static_cast<float>(BF_CHN_MAX));
...
void Visualizer::draw_target_text(cv::Mat resFrame, double time, float dis, float min_dis) {
    ...
    if (! _visdat->no_recent_brightness_events(time))
        warnings += "BRIGHTNESS ALERT";
    if (_patser->trackers.monster_alert())
        warnings += "ANOMOLY ALERT ";
    if (_fromfile)
        warnings += "REPLAY ";
...
void Visualizer::update_tracker_data(cv::Mat frameL, cv::Point3f setpoint, double time, bool draw_plots, bool draw_optimization) {
    enable_plots = draw_plots;
    enable_optimization_drawing = draw_optimization;
...
void Visualizer::draw_tracker_viz() {
    ...
    if (enable_optimization_drawing) {
        ...
        bool _via = _patser->interceptor.via();
        cv::Point3f _via_pos = _patser->interceptor.intermediate_pos();
        cv::Point2f _via_pos_im = world2im_2d(_via_pos, _visdat->Qfi, _visdat->camera_roll(), _visdat->camera_pitch());
        tracking::WorldItem wti = last_drone_detection.world_item;
        ...
        if (_patser->drone.control.at_base()) {
            ...
            if (_via) {
                cv::line(frameL_color, _pad_pos_im, _via_pos_im, red, 2);
                cv::line(frameL_color, _via_pos_im, _interception_pos_im, red, 2);
            }
            else
                cv::line(frameL_color, _pad_pos_im, _interception_pos_im, red, 2);
            cv::line(frameL_color, _pad_pos_im, _aim_pos_im, blue, 1);
        }
        else if (_patser->drone.control.ff_interception() || _patser->drone.control.flight_mode() == 11 || _patser->drone.control.flight_mode() == 12) {
            cv::line(frameL_color, _interception_pos_im, _stopping_pos_im, white, 1);
            if (_via) {
                cv::line(frameL_color, _drone_pos_im, _via_pos_im, red, 2);
                cv::line(frameL_color, _via_pos_im, _interception_pos_im, red, 2);
            }
            else
    ...
    if (_patser->trackers.diff_viz_buf.cols > 0) {
        ...
        if (pparams.op_mode == op_mode_x) {
            putText(diff, "Drone", cv::Point(3, diff.rows - 120), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 255, 0));
            putText(diff, "Drone template", cv::Point(3, diff.rows - 108), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(128, 0, 128));
            putText(diff, "Blink", cv::Point(3, diff.rows - 96), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 255));
        }
        ...
        putText(diff, "Multitracked", cv::Point(3, diff.rows - 60), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 128, 255));
        putText(diff, "False positive", cv::Point(3, diff.rows - 24), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 0));
        putText(diff, "Overexposed noise", cv::Point(3, diff.rows - 12), FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(128, 128, 128));
