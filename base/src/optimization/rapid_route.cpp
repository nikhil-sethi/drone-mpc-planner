...
rapid_route_result RapidRouteInterface::update_initial_guess(tracking::TrackData drone, tracking::TrackData target, rapid_route_result result) {
    ...
    float _estimated_time_to_intercept = largestRoot(1.f / 2.f * *_thrust, _velocity_error, -_position_error);
    ...
    cv::Point3f _estimated_drone_acceleration = -1 * _insect_direction * *_thrust;
...
rapid_route_result RapidRouteInterface::find_best_interception(tracking::TrackData drone, tracking::TrackData target) {
    ...
    int _iteration = 0;
    while (_iteration < 1000 && !(_iteration > 1 && norm(result.acceleration_to_intercept) > 0.99 * static_cast<double>(*_thrust) && norm(result.acceleration_to_intercept) < 1.01 * static_cast<double>(*_thrust))) {
        ...
        if (norm(result.acceleration_to_intercept) < static_cast<double>(*_thrust)) {
