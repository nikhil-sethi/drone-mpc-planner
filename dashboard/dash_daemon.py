...
Path(Path(pc.daemon_error_log).parent.absolute()).mkdir(parents=True, exist_ok=True)
file_format = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
error_file_handler = logging.handlers.RotatingFileHandler(filename=pc.daemon_error_log, maxBytes=1024 * 1024 * 100, backupCount=1)
...
def error_count():
    logger.error('rotate!')
    with open(pc.daemon_error_log, 'r') as f:
        lines = f.readlines
        return len(lines) - 1
