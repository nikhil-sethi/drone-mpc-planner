...
public:
    ...
    cv::Point2i drone_setpoint_im() {
        ...
        if (!drone_is_flying()) {
            tmp.x = _trackers->dronetracker()->drone_takeoff_location().x;
            tmp.y = _trackers->dronetracker()->drone_takeoff_location().y+0.5f;
            tmp.z = _trackers->dronetracker()->drone_takeoff_location().z;
        }
