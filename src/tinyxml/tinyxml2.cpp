...
{
    TIXML_SNPRINTF( buffer, bufferSize, "%f", static_cast<double>(v) );
}
    ...
    _compactMode( compact )
{
    ...
    for( int i=0; i<NUM_ENTITIES; ++i ) {
        ...
        if ( entities[i].value < ENTITY_RANGE ) {
            _entityFlag[ static_cast<int>(entities[i].value) ] = true;
        }
    }
    _restrictedEntityFlag[static_cast<int>('&')] = true;
    _restrictedEntityFlag[static_cast<int>('<')] = true;
    _restrictedEntityFlag[static_cast<int>('>')] = true;	// not required, but consistency is nice
    _buffer.Push( 0 );
}
    ...
    if ( _processEntities ) {
        while ( *q ) {
            ...
            if ( *q > 0 && *q < ENTITY_RANGE ) {
                ...
                if ( flag[static_cast<unsigned>(*q)] ) {
                    while ( p < q ) {
                        Print( "%c", *p );
                        ++p;
                    }
