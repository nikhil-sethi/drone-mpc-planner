...
void Realsense::check_light_level() {
    ...
    rs_dev.set_option(RS2_OPTION_GAIN,camparams.measured_gain);
    ...
    rs2::frameset frame;
    float new_expos;
    int nframes_delay = (fabs(camparams.measured_gain-tmp_set_gain)/248.f)*30.f;
    ...
    if (fabs(camparams.measured_exposure - new_expos) > 1000) {
        ...
        nframes_delay = (camparams.measured_gain/248.f)*60.f;
        ...
        while(true) {
            ...
            for (int i = 0; i< 60; i++) {
                frame = cam.wait_for_frames();
                if (frame.supports_frame_metadata(RS2_FRAME_METADATA_ACTUAL_EXPOSURE)) {
                    camparams.measured_exposure = frame.get_frame_metadata(rs2_frame_metadata_value::RS2_FRAME_METADATA_ACTUAL_EXPOSURE);
                    std::cout << "Exposure: " << tmp_measured_exposure<< " vs " << camparams.measured_exposure  << std::endl;
                    if (fabs(camparams.measured_exposure-tmp_measured_exposure)>0.5f) {
                        tmp_measured_exposure = camparams.measured_exposure;
            ...
            if (search_speed>0) {
                if (camparams.measured_exposure >= 15500 && camparams.measured_gain <= 248 ) {
                    camparams.measured_gain+=search_speed;
                    std::cout << " -> increasing gain: " << camparams.measured_gain << std::endl;
                    rs_dev.set_option(RS2_OPTION_GAIN, camparams.measured_gain);
                } else {
                    search_speed=-1;
                }
            ...
            if (search_speed<0) {
                if (camparams.measured_exposure < 15500 && camparams.measured_gain > 16 ) {
                    if (camparams.measured_gain > 248)
                        camparams.measured_gain = 248;
                    else
                        camparams.measured_gain+=search_speed;
                    std::cout << " -> decreasing gain: " << camparams.measured_gain << std::endl;
                    rs_dev.set_option(RS2_OPTION_GAIN, camparams.measured_gain);
                } else {
                    std::cout << "Measured exposure: " << camparams.measured_exposure << " gain: " << camparams.measured_gain << std::endl;
                    frameLt = Mat(im_size, CV_8UC1, const_cast<void *>(frame.get_infrared_frame(1).get_data()), Mat::AUTO_STEP);
                    break;
                }
            }
